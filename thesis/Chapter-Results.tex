...
% TODO: link to correct subsection.

Target variable is a magnetic interaction between two atoms measured by Nuclear Magnetic Resonance (NMR) - the so called scalar coupling constant or J-coupling constant.
% TODO add citation: NMR and Jcoupling
Not that in the graph notation, this is an edge-property as described in Section~\ref{sec:graph-output}. This interaction does not have to be predicted for any two atoms in the molecule but rather for a per-defined set of atom-pairs. These can be directly bonded to each other or be two or three covalent bonds apart. Furthermore, the can involve different atom types. The interactions are categorized by the involved atom types and the number of bonds between them. For instance, $2JHC$ is the J-coupling type of a hydrogen- and a carbon-atom with two bonds between them. % TODO: link EDA kernels


\subsection{Boosted Tree models with manually engineered features}
\label{sec:champs-boosted-tree}

Simple machine learning models based on manually engineered features, such as various versions of boosted trees can give surprisingly good results with short training time. They do not reach the performance of well tuned MPNNs but easily surpass not yet fully optimized MPNN models.

As the main topic of this thesis are MPNNs, this section will describe feature engineering approaches on a high level. 

\subsubsection{Distance and atom-type features}

The first type of engineered features were calculated in the following way: The center point between the two atoms whose J-coupling constant has to be predicted 
...
% TODO: add info about implementation and update
...
\subsection{Excluding hydrogen atoms from the molecule-graph speeds up training but reduces prediction accuracy}
